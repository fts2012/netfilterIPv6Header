...
* because of the capture task is done by libpcap, so there is no need to use netlink to communicate
* no need to use common too
*/
 ...
 * only the packets of measure info can send to userspace
 * TODO:how to deal with some info that not in measure but need communicate
 * here there is no need to use mmp to do other things
 */
static unsigned int 
